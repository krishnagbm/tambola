...
(function () {
  ...
  function getUserSession() {
    ...
    let userId = params.get('user_id') || hashParams.get('user_id') || localStorage.getItem('dabhousie_user_id') || '';
    let email = params.get('email') || hashParams.get('email') || localStorage.getItem('dabhousie_user_email') || '';
    let name = params.get('name') || hashParams.get('name') || localStorage.getItem('dabhousie_user_name') || '';
    let avatar = params.get('avatar') || hashParams.get('avatar') || localStorage.getItem('dabhousie_user_avatar') || '';
    let balance = params.get('balance') || hashParams.get('balance') || localStorage.getItem('dabhousie_balance') || '';
        ...
        for (let i = 0; i < localStorage.length; i++) {
          ...
          if (key && (key.includes('-auth-token') || key.includes('supabase.auth.token'))) {
            ...
            if (user) {
              ...
              name = name || user.user_metadata?.full_name || user.user_metadata?.name || email.split('@')[0];
              avatar = avatar || user.user_metadata?.avatar_url || user.user_metadata?.avatar || '';
              break;
            }
          }
        }
